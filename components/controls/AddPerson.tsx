...
const AddPerson = () => {
  ...
  const handleAddAvatar = () => {
    if (!data.height) {
      toast.error("Por favor, insira a altura");
      return;
    }
    ...
    <div className="w-full min-h-full space-y-3">
      ...
      {data.avatarCategory === AvatarCategory.CHILD && (
        <div>
          ...
          <TabStyleRadio
            options={["Garoto", "Garota"]}
            value={data.gender === Gender.MALE ? "Garoto" : "Garota"}
            onChange={(option) =>
              setState("gender", option === "Garoto" ? Gender.MALE : Gender.FEMALE)
            }
      ...
      {avatars.length < MAX_AVATARS ? (
        ...
        <Message variant="error">
          Máximo de {MAX_AVATARS} pessoas. Remova uma para adicionar outra.
        </Message>
      )}
      {/* Ad 2: Below the add button (PC and mobile) */}
      <AdsenseUnit slot="4881514671" format="auto" responsive={true} />
    </div>
  );
};
